...
from login.cam import VideoCamera, gen, add_face, playback
...
def reg(type, username, password, email, imgcode):
    ...
    if type == "user":
        add_user(username, password, email, tag)
    else:
        add_admin(username, password, email, tag)
...
def play_back():
    ...
    time = my_json[0]
    print(time)
    camera = cv2.VideoCapture('C:/Users/czh15/Desktop/ff/ca.mp4')
    return Response(playback(camera), content_type="multipart/x-mixed-replace; boundary=frame")
